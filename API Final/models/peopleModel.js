...
const createUser = async (user) => {
  ...
  try {
    ...
    const response = await conn.query(
      `INSERT INTO people(name, lastname, password, email) VALUES(?, ?, ?, ?, ?)`,
      [user.name, user.lastname, user.password, user.email]
    );
...
const updateUser = async (id, user) => {
  ...
  try {
    conn = await pool.getConnection();
    await conn.query(
      `UPDATE people SET name=?, lastname=?, password=?, email=? WHERE id=?`,
      [user.name, user.lastname, user.password, user.email, id]
    );
